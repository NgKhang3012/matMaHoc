...
char* read_pem_key(const char* filename) {
    ...
    int start = strstr(data, "-----BEGIN PRIVATE KEY-----") - data;
    start += (int)strlen("-----BEGIN PRIVATE KEY-----") + 1;
    // Tìm vị trí kết thúc của phần base64
    int end = strstr(data, "-----END PRIVATE KEY-----") - data;
...
__declspec(dllexport) int Sign(char* filepath, char* filepri, char* filesig)
{
    void* privkey, * sigpad;
    size_t privkey_len, sigpad_len, sig_len;
    uint8_t* tmpsd;
    FILE* file;
    size_t tmpsd_len;
    unsigned logn = 10;
    ...
    sigpad = xmalloc(sigpad_len);
    tmpsd_len = FALCON_TMPSIZE_SIGNDYN(logn);
    tmpsd = xmalloc(tmpsd_len);

    char* pdffile = readFromFile(filepath);
    privkey = base64_to_bytes(read_pem_key(filepri));
    sigpad_len = FALCON_SIG_PADDED_SIZE(logn);
    int r = falcon_sign_dyn(&rng, sigpad, &sigpad_len, FALCON_SIG_PADDED,
        privkey, privkey_len,
        pdffile, strlen(pdffile), tmpsd, tmpsd_len);
    if (r != 0) {
        return 1;
    }
    ...
    if (fopen_s(&file, filesig, "w") != 0) {
        return 1;
    }
    fprintf(file, "%s", bytes_to_base64(sigpad, sigpad_len));
    if (file == NULL) {
        return 1;
    }
    fclose(file);
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        printf("Usage: ");
        printf("%s ", argv[0]);
        printf("<filepdf> <file private key> <file sig>");
    }
    if (!Sign(argv[1], argv[2], argv[3])) {
        printf("Sucessfully");
    }
    else {
        printf("Fail");
